...
// Accesso al DB tramite Singleton
...
// 2. Gestione Export CSV
$download_link = '';
if (isset($_POST['export_stats']) && $corso_selezionato_id) {
    $filename = $csvExporter->exportStatisticheCorso($corso_selezionato_id);
    if ($filename) {
        $download_link = BASE_URL . 'exports/' . $filename;
    }
}

// 3. Calcolo Statistiche (Solo se un corso è selezionato)
$stats_base = null;
$distribuzione_voti = []; 
$tipi_valutazione = [];   
...
if ($corso_selezionato_id) {
    ...
    if($is_mio) {
        // A. Statistiche Base (Media, Min, Max) dalla classe Voto
        $stats_base = $votoObj->getStatisticheCorso($corso_selezionato_id);

        // B. Query Custom per Distribuzione Voti (Istogramma)
        ...
        // Inizializziamo l'array vuoto per coprire tutti i voti da 18 a 30
        ...
        // C. Query Custom per Tipi Valutazione (Torta)
        ...
        $corso_selezionato_id = ''; // Reset se non autorizzato
    }
}
...
<div class="container layout-contenuto">
    ...
    <?php if ($corso_selezionato_id && $stats_base): ?>
        ...
        <div class="griglia-statistiche mb-5">
            ...
            <div class="scheda-statistiche red">
